...
int32_t Message_ToByteArray(MESSAGE_HANDLE messageHandle, uint8_t* buf, size_t size)
{
    int32_t result;
    ...
    {
        LogError("invalid message handle (%p)", messageHandle);
        result = -1;
    }
    else if (buf == NULL && size > 0)
    {
        LogError("invalid buffer (%p)", buf);
        result = -1;
    }
    else
    {
